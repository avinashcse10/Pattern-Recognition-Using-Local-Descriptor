...
pattern_awslgs=awslgs_generator(pattern);
...
for i=0:window_length-1
    for j=0:window_breadth-1
     ...
     image_window_awslgs=awslgs_generator(image_window);
     window(:,:,i+1,j+1)=image_window_awslgs;
     dis(:,:,i+1,j+1)=image_window_awslgs-pattern_awslgs;
     dist=(dis.^2);
    dista=0;
     for k=1:pattern_size(1)
       for l=1:pattern_size(2)
